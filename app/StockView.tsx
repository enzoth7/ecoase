...
import { ArrowDownToLine, CalendarClock, ChevronRight, PackageCheck, RotateCcw, Search, ShieldAlert, SlidersHorizontal, TrendingDown, UsersRound } from "lucide-react";
...
type StockPagePayload = { stock: StockSummaryRow[]; providers: Provider[]; clients: Client[] };
...
async function requestStockPage(): Promise<StockPagePayload> {
  ...
  const [stockPayload, providersPayload, clientsPayload] = await Promise.all([stockResponse.json() as Promise<{ stock?: StockSummaryRow[]; error?: string }>, providersResponse.json() as Promise<{ providers?: Provider[] }>, clientsResponse.json() as Promise<{ clients?: Client[] }>]);
  if (!stockResponse.ok || !stockPayload.stock) throw new Error(stockPayload.error ?? "No se pudo cargar el stock.");
  return { stock: stockPayload.stock, providers: providersPayload.providers ?? [], clients: clientsPayload.clients ?? [] };
}
