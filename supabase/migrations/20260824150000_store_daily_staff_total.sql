drop function if exists public.upsert_capacity_daily_adjustment(date, text, text, text, text, integer, integer, text, text);
...
begin
  ...
  if p_resource_type <> 'internal_production' and p_pallet_adjustment = 0 then return null; end if;
